...
export default function PublicProfilePage() {
    ...
    fetch(`/api/profile/${profileId}`)
      .then(async (res) => {
        if (res.status === 410) {
          ...
          setProfile(await res.json() as Record<string, unknown>);
        }
      })
  ...
  if (expired) {
    return <RenewMessage profileId={profileId!} />;
  }
    ...
    return (
      <div className="min-h-dvh flex items-center justify-center px-4">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Profile Not Found</h1>
          <p className="text-gray-500 text-sm">The profile ID <span className="font-mono">{profileId}</span> does not exist.</p>
          <Link href="/" className="text-blue-700 text-sm mt-4 inline-block hover:underline">← Back to Home</Link>
        </div>
      </div>
    );
  }
...
function RenewMessage({ profileId }: { profileId: string }) {
  return (
      ...
      <div className="max-w-md w-full space-y-6 text-center">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm">
            ...
            Please contact Sunriver Systems to renew.
          </p>
        </div>
