...
import { Heart, Phone, MapPin, AlertCircle, User, Building2 } from "lucide-react";
...
export default function PublicProfileView({ profile }: Props) {
  ...
  const isOrg = ["Family", "School", "Business", "Institution"].includes(profile.accountType);
  ...
  return (
    <div className="min-h-dvh bg-gradient-to-br from-blue-50 to-white dark:from-gray-950 dark:to-gray-900">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <div className="flex justify-center gap-2 mt-2">
              <Badge>{profile.accountType}</Badge>
              ...
              <div className="flex items-center gap-3">
                ...
                {profile.health.height && (
                  <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                    <p>Height: <span className="font-medium text-gray-900 dark:text-gray-100">{profile.health.height}</span></p>
                    <p>Weight: <span className="font-medium text-gray-900 dark:text-gray-100">{profile.health.weight}</span></p>
                  </div>
                )}
              </div>
            </CardContent>
        ...
        {/* Medical conditions */}
        {profile.desperateConditions?.length > 0 && (
          <Card>
            <CardHeader className="pb-3">
            ...
            <CardContent className="space-y-3">
              {profile.desperateConditions.map((cond: any, i: number) => (
                <div key={i} className="p-3 bg-amber-50 dark:bg-amber-950 rounded-xl">
                  <p className="font-medium text-sm text-amber-900 dark:text-amber-100">
                    {cond.acuteCondition}
                  </p>
                  {cond.notes && (
                    <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">{cond.notes}</p>
                  )}
                  ...
                  {cond.hospital && (
                    <p className="text-xs text-gray-600 dark:text-gray-400 mt-1.5">
                      ...
                      {cond.hospitalContacts && ` · ${cond.hospitalContacts}`}
                    </p>
                  )}
                </div>
              ))}
        ...
        {profile.emergencyContacts?.length > 0 && (
          <Card>
            ...
            <CardContent className="space-y-3">
              {profile.emergencyContacts.slice(0, 3).map((contact: any, i: number) => (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {contact.mobile1 && (
                      <a
                        ...
                        className="text-xs text-blue-700 dark:text-blue-400 font-medium"
                      >
                        {contact.mobile1}
                      </a>
                    )}
            ...
            <CardContent>
              <p className="text-sm text-gray-700 dark:text-gray-300">
                {[
                  ...
                  profile.residence.ward,
                  profile.residence.district,
                  profile.residence.region,
                ]
                  .filter(Boolean)
                  .join(", ")}
        ...
        <div className="text-center py-4">
          <p className="text-xs text-gray-400">
            This profile is verified by TAARIFA_ID · Sunriver Systems
          </p>
          <p className="text-xs text-gray-400 mt-0.5">ID: {profile.profileId}</p>
        </div>
      </div>
    </div>
  );
