...
import {
  Plus, Lock, Unlock, RefreshCw, Users, X, Eye, EyeOff,
} from "lucide-react";
...
export default function SubAccountsManager({ session }: Props) {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm<CreateFormData>({
  ...
  async function doAction(subUserId: string, action: string, extra?: any) {
    setActionLoading(subUserId + action);
    try {
      const res = await fetch("/api/admin/sub-accounts", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subUserId, action, ...extra }),
      });
      const json = await res.json();
      if (!res.ok) { toast.error(json.message); return; }
    ...
    <div className="space-y-5">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-sm">
              <Plus size={14} /> New {accountType} Member
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <form onSubmit={handleSubmit(onCreate)} className="space-y-3">
              ...
              {nationality === "Tanzanian" && (
                <Input label="NIDA Number *" {...register("nidaNumber")} />
              )}
        ...
        <Card className="border-amber-200 dark:border-amber-800">
          <CardHeader>
            <CardTitle className="text-sm text-amber-700 dark:text-amber-300 flex items-center gap-2">
              <RefreshCw size={14} /> Reset Password for {resetTarget.firstName}
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0 space-y-3">
            ...
            <div className="flex gap-2">
              <Button
                size="sm"
                variant="warning"
                loading={actionLoading === resetTarget._id + "reset_password"}
                onClick={() => doAction(resetTarget._id, "reset_password", { newPassword: resetPwd })}
                disabled={resetPwd.length < 8}
              >
                Confirm Reset
              </Button>
              <Button size="sm" variant="ghost" onClick={() => { setResetTarget(null); setResetPwd(""); }}>
                Cancel
              </Button>
            </div>
          </CardContent>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400">
          ...
          <p className="text-sm">No sub-accounts yet. Add your first member above.</p>
        </div>
      ) : (
              ...
              <CardContent className="pt-4 pb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      ...
                      <Badge variant={user.isAccountActive ? "default" : "warning"}>
                        {user.isAccountActive ? "Paid" : "Pending"}
                      </Badge>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2 mt-3 flex-wrap">
                  <Button
                    ...
                    loading={actionLoading === user._id + (user.isActive ? "lock" : "unlock")}
                    onClick={() => doAction(user._id, user.isActive ? "lock" : "unlock")}
                  >
