...
import { ArrowRight, UserRound, HeartPulse, MapPin, Phone, ShieldAlert } from 'lucide-react';
...
export default function PublicProfilePage() {
  ...
  const main = data.profiles?.[0];
  const details: Array<[string, any]> = [];
  if (main) {
    for (const k of ['first_name', 'last_name', 'gender', 'birthdate', 'nationality', 'fluent_language']) {
      if (main[k] !== undefined) details.push([k.replace(/_/g, ' '), main[k]]);
    }
    if (main.health) {
      for (const k of ['blood_group', 'height', 'weight']) if (main.health[k]) details.push([`health: ${k}`, main.health[k]]);
    }
    if (main.residence) {
      for (const k of ['region', 'district', 'ward', 'street']) if (main.residence[k]) details.push([k, main.residence[k]]);
    }
  }
          ...
          <div className="relative">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="mt-0.5 text-[13px] capitalize text-ink-secondary">{data.account_type} profile</div>
              </div>
              {data.qr_data_url && <img src={data.qr_data_url} alt="QR code" className="h-24 w-24 rounded-xl" />}
            </div>

            <div className="mt-5 rounded-button bg-glass-subtle px-4 py-3 text-center font-mono text-[16px] font-semibold tracking-wider text-ink-primary">
              {data.profile_id}
            </div>
            ...
            {main?.emergency_contacts?.length > 0 && (
            ...
            {main?.desperate_conditions?.length > 0 && (
